...
create or replace package body BUSINESS_GROUP_INTF_PCK is
  ...
  begin
    ...
    exception
      ...
      pro_exe_log('P_PROCESS_0X21', 'log', 'log', v_dataInfo);
  ...
  procedure make_lms(v_messageType in varchar2,
    ...
    iv_charIndex              number(12);
    iv_sampleType             varchar2(30);   --货物方向
  ...
  begin
   ...
   --查找LMS202堆存数据信息 
    ...
    iv_charIndex:=instr(iv_bolNo,'-');
   
    if iv_charIndex>1 then
      iv_bolNo:= substr(iv_bolNo, 0,iv_charIndex-1);
    end if;
    ...
    begin
     ...
     if iv_property in('WM') and iv_impexp in('I') and iv_sampleType in ('0') then---外贸进口 出库才发送堆存报文
