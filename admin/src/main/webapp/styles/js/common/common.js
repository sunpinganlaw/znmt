...
function toLocaleDateString(value)
{
    if(value) {
...
function printIntelCtrl(objName) {
    $(objName).printThis();
}
function hiddenAndChange(area,type,classname)
{
    var inputArray = $(area+" "+type+"[class='"+classname+"']");
    inputArray.each(//使用数组的循环函数 循环这个input数组
        function () {
            var input = $(this);//循环中的每一个input元素
            input.after("<label><i>" + input.val() + "</i></label>");
            input.remove();
        }
    )
}
function selectAll(divname,type)
{
    var inputArray = $("#"+divname+" input[id*='"+type+"']");
    var this_M = $("#"+divname+" #"+type);

    if(this_M.is(':checked')) {
        inputArray.each(//使用数组的循环函数 循环这个input数组
            function () {
