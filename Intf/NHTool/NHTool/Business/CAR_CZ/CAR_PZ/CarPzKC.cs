...
namespace NHTool.Business.CAR_CZ.CAR_PZ
{
    class CarPzKC : CarCzBusiness
    {
        ...
        public override Boolean initial()
        {
            ...
            {
                LogTool.WriteLog(typeof(CarPzKC), "亚当模块初始化失败");
            ...
            {
                LogTool.WriteLog(typeof(CarPzKC), "前RFID初始化失败");
            ...
            {
                LogTool.WriteLog(typeof(CarPzKC), "声音模块初始化失败");
            ...
            {
                LogTool.WriteLog(typeof(CarPzKC), "LED初始化失败");
            ...
            {
                LogTool.WriteLog(typeof(CarPzKC), "地磅串口初始化失败");
        ...
        public override void comPort_DataReceived(object sender, SerialDataReceivedEventArgs e)
        {
            try
            {
                if (Commons.getDcValue(ctlConfig, "currentFlowId").Equals("HP2"))
                {
                ...
                else
                {
                    ...
                    if (_byteData.Count >= 12)
                    {
                        ...
                        for (int i = 0; i < _byteData.Count; i++)
                        {
                         ...
                         string readString = Encoding.ASCII.GetString(readBuffer, 0, readBuffer.Length);
                         string weightStr = readString.Substring(2,6);//---取得表头重量的所有数字
                         string dotIndex = readString.Substring(8, 1);//---取得小数点的从右往左的位数
                         int Index=0;
                         int.TryParse(dotIndex, out Index);
                ...
                new Thread(new ThreadStart(delegate
                {
                    ...
                    while (true)
                    {
                        try
                        {
                            ...
                            LogTool.WriteLogInfo(typeof(CarPzKC), "currentStep=" + stateEventArgs.nextStep.ToString());
                            ...
                            switch (stateEventArgs.nextStep)
                            {
                                ...
                                case Commons.STEP.CAR_WeightFinish: weightFinishStep("PZ"); break;//继承时，具体重载指定
                        ...
                        {
                            LogTool.WriteLog(typeof(CarPzKC), "主循环异常:" + e.Message);
                        }
                    }
                })).Start();
            }
        ...
        public override Boolean invokeDbCzBefore(string czType)
        {
            ...
            if ("MZ".Equals(czType))
            {
                ...
                LogTool.WriteLogInfo(typeof(CarCzBusiness), "invokeDbCzBefore,Mz,require dataJson=" + dataJson.ToString());
            ...
            else if ("PZ".Equals(czType))
            {
                ...
                LogTool.WriteLogInfo(typeof(CarPzKC), "invokeDbCzBefore,Pz,require dataJson=" + dataJson.ToString());
        ...
        public override void weightFinishStep(String czType)
        {
            ...
            if ("MZ".Equals(czType))
            {
                ...
                LogTool.WriteLogInfo(typeof(CarPzKC), "invokeDbCzFinish,Mz,require dataJson=" + dataJson.ToString());
            }
            else if ("PZ".Equals(czType))
            {
                dataJson.Add("cardId", Convert.ToString(stateEventArgs.cardID));
                ...
                LogTool.WriteLogInfo(typeof(CarPzKC), "invokeDbCzFinish,Pz,require dataJson=" + dataJson.ToString());
            }
