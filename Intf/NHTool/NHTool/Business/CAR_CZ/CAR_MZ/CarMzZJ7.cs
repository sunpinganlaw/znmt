...
{
    class CarMzZJ7 : CarCzBusiness
    {
        public string channelNo;
        string newMineCardNo;
        ...
        public override Boolean initial()
        {
            ...
            {
                LogTool.WriteLog(typeof(CarMzZJ7), "亚当模块初始化成功【IP：" + ctlConfig["ADAM_IP"]+"】");
                initCount++;
            }
            else
            {
                LogTool.WriteLog(typeof(CarMzZJ7), "亚当模块初始化失败");
            ...
            {
                LogTool.WriteLog(typeof(CarMzZJ7), "天线初始化成功【IP：" + ctlConfig["RFID_IP"] + "】");
                initCount++;
            }
            else
            {
                LogTool.WriteLog(typeof(CarMzZJ7), "前RFID初始化失败");
            ...
            {
                LogTool.WriteLog(typeof(CarMzZJ7), "LED初始化成功【IP：" + ctlConfig["LED_IP"] + "】");
                initCount++;
            }
            else
            {
                LogTool.WriteLog(typeof(CarMzZJ7), "LED初始化失败");
            ...
            {
                LogTool.WriteLog(typeof(CarMzZJ7), "地磅串口初始化成功！");
                initCount++;
            }
            else
            {
                LogTool.WriteLog(typeof(CarMzZJ7), "地磅串口初始化失败");
            ...
            {
                initResult = true;
                LogTool.WriteLog(typeof(CarMzZJ7), "初始化成功 " + initCount + "/" + expectDeviceCount + " 个设备");
            }
            else 
            {
                LogTool.WriteLog(typeof(CarMzZJ7), "初始化失败：启动 " + initCount + "/" + expectDeviceCount + " 个设备");
                ...
                new Thread(new ThreadStart(delegate
                {
                    ...
                    while (true)
                    {
                        ...
                        try
                        {
                            ...
                            //LogTool.WriteLog(typeof(CarMzZJ7), "currentStep=" + stateEventArgs.nextStep.ToString());
                            ...
                            switch (stateEventArgs.nextStep)
                            {
                                ...
                                case Commons.STEP.CAR_GetMineCard: getMineInfoStep(90); break; //获取矿卡信息
        ...
        public void getMineInfoStep(int seconds)
        {
            ...
            for (int i = 0; i < seconds; i++)
            {
                ...
                if (newMineCardNo != null && !newMineCardNo.Equals(""))
                {
                    writeMointorSingle("矿卡号", newMineCardNo);
                    stateEventArgs.mineCardID = newMineCardNo;
        ...
        public Boolean invokeDBCzBefore()
        {
            ...
            {
                LogTool.WriteLog(typeof(CarMzZJ7), e.StackTrace);
                LogTool.WriteLog(typeof(CarMzZJ7), "invokeDBCzBefore数据库调用失败：" + e.Message);
                retJson = new JObject();
        ...
        public Boolean invokeDBCzAfter()
        {
            ...
            {
                LogTool.WriteLog(typeof(CarMzZJ7), e.StackTrace);
                LogTool.WriteLog(typeof(CarMzZJ7), "invokeDBCzAfter数据库调用失败：" + e.Message);
                retJson = new JObject();
                retJson.Add("logicRetCode", "1999");
                retJson.Add("logicRetMsg", "数据库调用失败");
                retJson.Add("resMsg", "数据库调用失败");
        ...
        public void localWeightBackUp()
        {
            ...
            {
                LogTool.WriteLog(typeof(CarMzZJ7), "localWeightBackUp数据库调用失败：" + e.Message);
            }
        }
        ...
        public JObject invokeDBCheckCarCard()
        {
            ...
            {
                LogTool.WriteLog(typeof(CarMzZJ7), e.StackTrace);
                LogTool.WriteLog(typeof(CarMzZJ7), "invokeDBCheckCarCard数据库调用失败：" + e.Message);
                retJson = new JObject();
                retJson.Add("logicRetCode", "1999");
                retJson.Add("logicRetMsg", "数据库调用失败");
                retJson.Add("resMsg", "数据库调用失败");
        ...
        public JObject invokeDBCheckMineCard()
        {
            ...
            {
                LogTool.WriteLog(typeof(CarMzZJ7), e.StackTrace);
                LogTool.WriteLog(typeof(CarMzZJ7), "invokeDBCheckMineCard数据库调用失败：" + e.Message);
                retJson = new JObject();
                retJson.Add("logicRetCode", "1999");
                retJson.Add("logicRetMsg", "数据库调用失败");
                retJson.Add("resMsg", "数据库调用失败");
        ...
        //委托处理事务获取到卡号的处理
        public override void processHFCardID(string cardId)
        {
            newMineCardNo = cardId;
        }
