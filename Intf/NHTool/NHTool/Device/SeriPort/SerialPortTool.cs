...
namespace NHTool.Device.SeriPort
 
{
    public class SerialPortTool
    {
        ...
        public delegate void serialPortDeviceHander(byte[] myReceivedData);
        public event serialPortDeviceHander serialPortDeviceEvent;
        /// <summary>
        /// 串口号
        /// </summary>
        ...
        public SerialPortTool(string name, SerialPortBaudRates baud, Parity par, SerialPortDatabits dBits, StopBits sBits)
        {
            _portName = name;
            _baudRate = baud;
            _parity = par;
        ...
        /// <param name="dBits">数据位</param>
        /// <param name="name">串口号</param>
        public SerialPortTool(string name, string baud, string par, string dBits, string sBits)
        {
            _portName = name;
         ...
         void comPort_DataReceived(object sender, SerialDataReceivedEventArgs e)
        {
            ...
            //    {
            //        DataReceived(new DataReceivedEventArgs(Convert.ToString(weight)));
            //    }
            //}
        }

        private void comPort_DataReceived_New()
        {
            while (true)
            {
                byte[] readBuffer = new byte[comPort.ReadBufferSize + 1];
                ...
                if (count > 0)
                {
                    serialPortDeviceEvent(readBuffer);
                }
