		...
		this.each(function() {
			...
			input.bind("keyup", function(e){
				if(e.which != 13 && e.which != 27 
					...
					proposalList.empty();
					if(input.val() != ''){
						var word = "^" + input.val() + ".*"+params.suffix;
						proposalList.empty();
						for(var i= 0,len=params.hints.length;i<len;i++){
							if(params.hints[i].match(word)){
								currentProposals.push(params.hints[i]);
								var element = $('<li></li>')
									.html(params.hints[i])
									.addClass('proposal')
									.click(function(){
			...
			if(params.showButton){
				...
				var button = $('<div></div>')
					...
					.click(function(){
						proposalList.empty();
						if($.inArray(input.val(),params.hints)!=-1){
							params.onSubmit(input.val());
						}else{
							input.val("");
						}
